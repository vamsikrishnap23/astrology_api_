...
from .calculations import (
    ...
    get_rasi,
    get_ascendant,
    get_tropical_ascendant,
    get_ayanamsa
)

def _is_odd_sign(sign):
    # sign: 0-based (0=Aries)
    return sign % 2 == 0

def _is_movable_sign(sign):
    # 0: Aries, 3: Cancer, 6: Libra, 9: Capricorn
    return sign % 3 == 0

def _is_fixed_sign(sign):
    # 1: Taurus, 4: Leo, 7: Scorpio, 10: Aquarius
    return sign % 3 == 1

def _is_dual_sign(sign):
    # 2: Gemini, 5: Virgo, 8: Sagittarius, 11: Pisces
    return sign % 3 == 2
...
def compute_planets_in_varga(
    year, month, day, hour, minute, second, lat, lon, tz_offset, varga_num
):
...
def compute_planets_in_bhava(
    year, month, day, hour, minute, second, lat, lon, tz_offset
):
    swe.set_sid_mode(swe.SIDM_LAHIRI)
    jd = get_julian_day(year, month, day, hour, minute, second, tz_offset)
    ayanamsa = get_ayanamsa(jd)
